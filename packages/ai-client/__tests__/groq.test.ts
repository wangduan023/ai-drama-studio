...
describe('GroqClient', () => {
  ...
  beforeEach(() => {
    client = new GroqClient(defaultConfig)
  ...
  describe('constructor', () => {
    it('应该使用默认 baseURL 初始化', () => {
      const clientWithDefaults = new GroqClient({
        provider: 'groq',
        modelId: 'llama-3.1-70b-versatile',
        apiKey: 'test-key',
      })
    ...
    it('应该使用自定义 baseURL 初始化', () => {
      const customBaseURL = 'https://custom.groq.api'
      const clientWithCustomURL = new GroqClient({
        provider: 'groq',
        modelId: 'llama-3.1-70b-versatile',
        apiKey: 'test-key',
  ...
  describe('generateText', () => {
    ...
    it('应该处理 Groq 特有的 x_groq usage 格式', async () => {
      const mockChunks = [
        'data: {"choices": [{"delta": {"content": "Hello"}}], "x_groq": {"usage": {"prompt_tokens": 10, "completion_tokens": 20, "total_tokens": 30}}}\n',
        'data: [DONE]\n',
      ]
      let chunkIndex = 0
  ...
  describe('generateImage', () => {
    it('应该返回不支持的错误', async () => {
      ...
      expect(result.error).toBe('Groq 不支持图像生成')
    })
  })
  ...
  describe('generateVideo', () => {
    it('应该返回不支持的错误', async () => {
      ...
      expect(result.success).toBe(false)
      expect(result.error).toBe('Groq 不支持视频生成')
    })
  })
  ...
  describe('generateAudio', () => {
    it('应该返回不支持的错误', async () => {
      ...
      expect(result.success).toBe(false)
      expect(result.error).toBe('Groq 不支持语音生成')
    })
  })
})
