...
describe('BaichuanClient', () => {
  ...
  const defaultConfig: AIModelConfig = {
    provider: 'baichuan',
    modelId: 'Baichuan4',
    apiKey: 'test-baichuan-key',
    baseURL: 'https://api.baichuan-ai.com/v1',
    timeout: 5000,
  }

  beforeEach(() => {
    client = new BaichuanClient(defaultConfig)
  ...
  describe('constructor', () => {
    it('应该使用默认 baseURL 初始化', () => {
      const clientWithDefaults = new BaichuanClient({
        provider: 'baichuan',
        modelId: 'Baichuan4',
        apiKey: 'test-key',
      })
    ...
    it('应该使用自定义 baseURL 初始化', () => {
      const customBaseURL = 'https://custom.baichuan.api'
      const clientWithCustomURL = new BaichuanClient({
        provider: 'baichuan',
        modelId: 'Baichuan4',
        apiKey: 'test-key',
    ...
    it('应该正确设置 provider', () => {
      expect(client.provider).toBe('baichuan')
    })
  })
  ...
  describe('generateImage', () => {
    it('应该返回不支持的错误', async () => {
      ...
      expect(result.error).toContain('百川智能客户端不支持图像生成')
    })
  })
  ...
  describe('generateVideo', () => {
    it('应该返回不支持的错误', async () => {
      ...
      expect(result.success).toBe(false)
      expect(result.error).toBe('百川智能不支持视频生成')
    })
  })
  ...
  describe('generateAudio', () => {
    it('应该返回不支持的错误', async () => {
      ...
      expect(result.success).toBe(false)
      expect(result.error).toBe('百川智能不支持语音生成')
    })
  })
})
