...
describe('IflytekClient', () => {
  let client: IflytekClient
  let mockFetch: ReturnType<typeof vi.spyOn>

  const defaultConfig: AIModelConfig = {
    provider: 'iflytek',
    modelId: 'v3.5',
    apiKey: 'test-api-key:test-api-secret',
    baseURL: 'wss://spark-api.xf-yun.com',
    timeout: 5000,
  }

  beforeEach(() => {
    client = new IflytekClient(defaultConfig)
  ...
  describe('constructor', () => {
    it('应该使用默认 baseURL 初始化', () => {
      const clientWithDefaults = new IflytekClient({
        provider: 'iflytek',
        modelId: 'v3.5',
        apiKey: 'test-api-key:test-api-secret',
      })
    ...
    it('应该使用自定义 baseURL 初始化', () => {
      const customBaseURL = 'https://custom.iflytek.api'
      const clientWithCustomURL = new IflytekClient({
        provider: 'iflytek',
        modelId: 'v3.5',
        apiKey: 'test-api-key:test-api-secret',
    ...
    it('应该正确设置 provider', () => {
      expect(client.provider).toBe('iflytek')
    })
  })
  ...
  describe('generateText', () => {
    ...
    it('应该处理空响应', async () => {
      ...
      ).rejects.toThrow('星火未返回任何内容')
    }, 20000)

    it('应该在 API Key 格式错误时抛出错误', async () => {
      const invalidClient = new IflytekClient({
        provider: 'iflytek',
        modelId: 'v3.5',
      ...
      ).rejects.toThrow('讯飞 API Key 格式错误')
    })
  ...
  describe('generateImage', () => {
    it('应该返回不支持的错误', async () => {
      ...
      expect(result.error).toBe('星火不支持图像生成')
    })
  })
  ...
  describe('generateVideo', () => {
    it('应该返回不支持的错误', async () => {
      ...
      expect(result.success).toBe(false)
      expect(result.error).toBe('星火不支持视频生成')
    })
  })
  ...
  describe('generateAudio', () => {
    it('应该返回不支持的错误', async () => {
      ...
      expect(result.success).toBe(false)
      expect(result.error).toContain('星火客户端不支持语音生成')
    })
  })
})
