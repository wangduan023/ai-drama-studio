...
describe('MoonshotClient', () => {
  ...
  beforeEach(() => {
    client = new MoonshotClient(defaultConfig)
  ...
  describe('constructor', () => {
    it('应该使用默认 baseURL 初始化', () => {
      const clientWithDefaults = new MoonshotClient({
        provider: 'moonshot',
        modelId: 'moonshot-v1-8k',
        apiKey: 'test-key',
      })
    ...
    it('应该使用自定义 baseURL 初始化', () => {
      const customBaseURL = 'https://custom.moonshot.api'
      const clientWithCustomURL = new MoonshotClient({
        provider: 'moonshot',
        modelId: 'moonshot-v1-8k',
        apiKey: 'test-key',
  ...
  describe('generateImage', () => {
    it('应该返回不支持的错误', async () => {
      ...
      expect(result.error).toBe('Kimi 不支持图像生成')
    })
  })
  ...
  describe('generateVideo', () => {
    it('应该返回不支持的错误', async () => {
      ...
      expect(result.success).toBe(false)
      expect(result.error).toBe('Kimi 不支持视频生成')
    })
  })
  ...
  describe('generateAudio', () => {
    it('应该返回不支持的错误', async () => {
      ...
      expect(result.success).toBe(false)
      expect(result.error).toBe('Kimi 不支持语音生成')
    })
  })
})
