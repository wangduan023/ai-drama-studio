...
export class GroqClient extends BaseAIClient {
  ...
  async generateText(
    ...
    return this.withRetry(async () => {
      ...
      const body: Record<string, unknown> = {
        model: this.modelId || 'llama-3.1-70b-versatile',
          ...
          try {
            ...
            if (choices && choices.length > 0) {
              ...
              if (data.x_groq?.usage) {
                ...
                usage = {
                  promptTokens: Number(groqUsage.prompt_tokens) || 0,
                  completionTokens: Number(groqUsage.completion_tokens) || 0,
                  totalTokens: Number(groqUsage.total_tokens) || 0,
                }
              } else if (data.usage) {
                usage = {
  ...
  private parseTextResponse(data: Record<string, unknown>): TextGenerateResult {
    const choices = data.choices as Array<Record<string, unknown>> | undefined
    if (!choices || choices.length === 0) {
      throw createAIError('EMPTY_RESPONSE', 'Groq 未返回任何内容', {
    ...
    if (usageData) {
      usage.promptTokens = Number(usageData.prompt_tokens) || 0
      usage.completionTokens = Number(usageData.completion_tokens) || 0
      usage.totalTokens = Number(usageData.total_tokens) || 0
    }
